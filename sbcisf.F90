...
!: Variable used in fldread to read the forcing file (nn_isf == 4 .OR. nn_isf == 3)
   ...
   TYPE(FLD_N)       , PUBLIC ::   sn_qisf, sn_fwfisf     !: information about the runoff file to be read !Yona not used in my case (nn_isf=3)
    ...
    REAL(wp)                            :: zhisf
      ...
      IF( MOD( kt-1, nn_fsbc) == 0 ) THEN
         ...
         ELSE IF (nn_isf == 3) THEN
           ...
           IF( ln_flx_ano .AND. ln_fwf_ano ) fwfisf(:,:) = fwfisf(:,:) + fwisf_ano(:,:) !Yona add freshwater flux anomaly
           IF( ln_flx_ano .AND. ln_heat_ano ) qisf(:,:)   = qisf(:,:)   + hfisf_ano(:,:) !Yona add heat flux anomaly
         ...
         ELSE IF (nn_isf == 4) THEN
            ...
            ! Yona : adding case nn_isf = 5 to read heaflux and freshwater flux from iceshelf
          ELSE IF (nn_isf == 5) THEN
            ! specified fwf and heat flux forcing beneath the ice shelf
            IF( .NOT.l_isfcpl ) THEN
               CALL fld_read ( kt, nn_fsbc, sf_qisf   )
               CALL fld_read ( kt, nn_fsbc, sf_fwfisf   )
               fwfisf(:,:) = -sf_fwfisf(1)%fnow(:,:,1)            ! fwf ! Yona : rajout d'un '-' car iceshelf_cea = -fwfisf
               qisf(:,:) = sf_qisf(1)%fnow(:,:,1)            ! heat flux
               IF( ln_flx_ano .AND. ln_fwf_ano ) fwfisf(:,:) = fwfisf(:,:) + fwisf_ano(:,:)
               IF( ln_flx_ano .AND. ln_heat_ano ) qisf(:,:)   = qisf(:,:)   + hfisf_ano(:,:)
            ELSE
               qisf(:,:)   = fwfisf(:,:) * lfusisf              ! heat        flux
               !qisf(:,:)   = sf_qisf(1)%fnow(:,:,1)              ! heat flux
            ENDIF
...
!         zpress = grav*rau0*fsdept(ji,jj,jk)*1.e-04
         ...
         ! Yona add case nn_isf=5 : qisf read = latent+sensible vs. nn_isf!=5 qisf =latent, so add sensible part
         IF (nn_isf /= 5) THEN
