...
CONTAINS
   ...
   SUBROUTINE tra_adv_tvd ( kt, kit000, cdtype, p2dt, pun, pvn, pwn,      &
      ...
      LOGICAL ,                    OPTIONAL, INTENT(in)    ::   ld_dia          ! computation of trends diag
      !
      ...
      IF( kt == kit000 )  THEN
         IF(lwp) WRITE(numout,*)
         IF( PRESENT( ld_dia ) )  THEN
            IF(lwp) WRITE(numout,*) 'tra_adv_tvd : TVD advection scheme on ', cdtype, ' for eiv trends diagnostics only'
         ELSE
      ...
      DO jn = 1, kjpt                                            ! tracer loop
         ...
         IF( l_trans .AND. jn==jp_tem ) THEN
            ...
            END DO
             ...
             CALL iom_put( "uadv_heattr",  z2d )       ! heat transport in i-direction
              !
            z2d(:,:) = 0._wp 
            DO jk = 1, jpkm1
               DO jj = 2, jpjm1
            ...
            CALL iom_put( "vadv_heattr", z2d )       ! heat transport in j-direction
         ENDIF
         ! "Poleward" heat and salt transports (contribution of upstream fluxes)
         IF( cdtype == 'TRA' .AND. ln_diaptr ) THEN  
