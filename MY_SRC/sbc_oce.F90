MODULE sbc_oce
   ...
   LOGICAL , PUBLIC ::   ln_flx_ano     !: flux      formulation to read the anomalies !Yona
   ...
   REAL(wp), PUBLIC, ALLOCATABLE, SAVE, DIMENSION(:,:) ::   atm_n2o           !: atmospheric pN2O                             [ppm]

   REAL(wp), PUBLIC, ALLOCATABLE, SAVE, DIMENSION(:,:,:) :: xcplmask          !: coupling mask for ln_mixcpl (warning: allocated in sbccpl)

   !!clem !Yona
   REAL(wp), PUBLIC, ALLOCATABLE, SAVE, DIMENSION(:,:) ::   vcpl_i             !: ice volume from the coupled simu
   REAL(wp), PUBLIC, ALLOCATABLE, SAVE, DIMENSION(:,:) ::   hcpl_i             !: ice thickness from the coupled simu
   !!clem
   ...
   REAL(wp), PUBLIC, ALLOCATABLE, SAVE, DIMENSION(:,:) ::   qfrz_m     !: mean (nn_fsbc time-step) supercooling heat flux [W/m2]
...
CONTAINS
   ...
   INTEGER FUNCTION sbc_oce_alloc()
      ...
      ALLOCATE( tprecip(jpi,jpj) , sprecip(jpi,jpj) , fr_i(jpi,jpj) ,     &
         &      atm_co2(jpi,jpj) , atm_n2o(jpi,jpj) , vcpl_i(jpi,jpj), hcpl_i(jpi,jpj),                   &
         &      ssu_m  (jpi,jpj) , sst_m(jpi,jpj) , frq_m(jpi,jpj) , qfrz_m(jpi,jpj) ,     &
