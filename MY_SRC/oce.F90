MODULE oce
   ...
   !!clemfra
   REAL(wp), PUBLIC, ALLOCATABLE, SAVE, DIMENSION(:,:,:) ::   fraqsr             !: fraction of solar net radiation absorbed in the k-th ocean level [-]

   !!Yona
   REAL(wp), PUBLIC, ALLOCATABLE, SAVE, DIMENSION(:,:) ::   qfrz    !!clem: supercooling heat flux (over mixing layer depth) [W/m2]
   !!----------------------------------------------------------------------
...
CONTAINS
   ...
   INTEGER FUNCTION oce_alloc()
      ...
      ALLOCATE( snwice_mass(jpi,jpj) , snwice_mass_b(jpi,jpj), snwice_fmass(jpi,jpj) , STAT=ierr(3) )
         !
      ALLOCATE( fraqsr_1lev(jpi,jpj) , qfrz(jpi,jpj) , STAT=ierr(4) )
         !
      ALLOCATE( fraqsr(jpi,jpj,jpk) , STAT=ierr(5) )
      !
