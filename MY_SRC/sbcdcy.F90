...
CONTAINS
   ...
   FUNCTION sbc_dcy( pqsrin, l_mask, kday_qsr ) RESULT( zqsrout )
      !!----------------------------------------------------------------------
      !!                  ***  ROUTINE sbc_dcy  ***
      !!
      ...
      INTEGER, OPTIONAL, INTENT(in) :: kday_qsr ! use the routine for night mask computation
      ...
      !                                          
      IF( iday_qsr == -1 ) THEN       ! first time step only  
         IF(lwp) THEN
            WRITE(numout,*)
