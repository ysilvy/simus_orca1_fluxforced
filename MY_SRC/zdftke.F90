...
#endif
!CE (!Yona je crois que ce bout a été rajouté)
#if defined key_lim3
...
!!clem #if defined key_lim3
            !zmxlm(:,:,1) = ( ( 1. - fr_i(:,:) ) * zraug * taum(:,:) + fr_i(:,:) * hcpl_i(:,:) * 2. ) * tmask(:,:,1)
            zmxlm(:,:,1) = ( ( 1. - fr_i(:,:) ) * zraug * taum(:,:) + vcpl_i(:,:) * 2. ) * tmask(:,:,1) ! Yona use either vcpl_i or fr_i*hcpl_i
...
!!clem #endif
         ...
         CASE( 3 )                                 ! scaling with max sea-ice thickness
            ...
            zmxlm(:,:,1) = ( ( 1. - fr_i(:,:) ) * zraug * taum(:,:) + fr_i(:,:) * MAXVAL( ht_i, 3 ) ) * tmask(:,:,1)
#else
            CALL ctl_stop( 'STOP', 'zdf_tke : you are trying to use option nn_xml0=3 while you do not have ice!! Silly newbie' )
!!clem            zmxlm(:,:,1) = zraug * taum(:,:) * tmask(:,:,1)
#endif
            !
         END SELECT
         !
