...
#  include "domzgr_substitute.h90"
   ...
   !! $Id: tranxt.F90 9272 2018-01-20 08:23:14Z gm $
...
CONTAINS
   ...
   SUBROUTINE tra_nxt( kt )
      ...
      !!
      INTEGER  ::   ji, jj, jk, jn    ! dummy loop indices !!clem added ji, jk
      REAL(wp) ::   zfact     ! local scalars
      REAL(wp), POINTER, DIMENSION(:,:,:) ::  ztrdt, ztrds
      REAL(wp), DIMENSION(jpi,jpj) ::  zt_frz !!clem
      !!----------------------------------------------------------------------
      !
      ...
      IF( ln_frz ) THEN
         DO jk = 1, jpkm1
            CALL eos_fzp(tsa(:,:,jk,jp_sal), zt_frz(:,:), gdept_n(:,:,jk)) ! freezing temperature [Celcius]
            DO jj = 1, jpj
               DO ji = 1, jpi
                  IF( tsa(ji,jj,jk,jp_tem) < zt_frz(ji,jj)+rn_frzup .AND. jk < nmln(ji,jj) ) THEN
                     qfrz(ji,jj) = qfrz(ji,jj) + rau0_rcp * ( zt_frz(ji,jj)+rn_frzup - tsa(ji,jj,jk,jp_tem) ) * e3t_a(ji,jj,jk) &
                        &                                                                  / MAX( r2dtra(jk), rn_dtfrz*86400. ) ! W/m2
                     tsa(ji,jj,jk,jp_tem) = tsa(ji,jj,jk,jp_tem) + ( zt_frz(ji,jj)+rn_frzup - tsa(ji,jj,jk,jp_tem) )            &
                        &                                                     * r2dtra(jk) / MAX( r2dtra(jk), rn_dtfrz*86400. )
                  ENDIF
               ENDDO
            ENDDO
         ENDDO
      ENDIF
      CALL iom_put( "qfrz" , qfrz )
      !!clem
