...
#  include "domzgr_substitute.h90"
   ...
   !! $Id: sbcrnf.F90 9567 2018-05-10 14:26:43Z mathiot $
...
CONTAINS
   ...
   SUBROUTINE sbc_rnf( kt )
      ...
      IF( MOD( kt - 1, nn_fsbc ) == 0 ) THEN
         !
         IF( .NOT. l_rnfcpl ) THEN
             rnf(:,:) = rn_rfact * ( sf_rnf(1)%fnow(:,:,1) ) * tmask(:,:,1)  ! updated runoff value at time step kt !Yona mask
   !! chris
             CALL lbc_lnk( rnf(:,:), 'T',  1. ) !Yona
   !! chris
             IF( ln_rnf_icb ) THEN
                ...
                CALL iom_put( 'hflx_icb_cea' , fwficb(:,:) * Lfus )   ! output Heat Flux into Sea Water due to Iceberg Thermodynamics -->
             ENDIF
             IF( ln_flx_ano .AND. ln_fwf_ano ) rnf(:,:) = rnf(:,:) + fwrnf_ano(:,:) !Yona
         ENDIF
         ...
         IF( ln_rnf_tem ) THEN                                       ! use runoffs temperature data
            ...
            rnf_tsc(:,:,jp_tem) = ( sf_t_rnf(1)%fnow(:,:,1) * tmask(:,:,1) ) * r1_rau0_rcp ! Yona: changed this line to read heatflux instead of temperature, and commented all the lines below
   !! chris
            CALL lbc_lnk( rnf_tsc(:,:,jp_tem), 'T',  1. ) !Yona
   !! chris
            IF( ln_flx_ano .AND. ln_heat_ano ) rnf_tsc(:,:,jp_tem) = rnf_tsc(:,:,jp_tem) + hfrnf_ano(:,:) * r1_rau0_rcp !Yona
         ...
         ELSE                                                        ! use SST as runoffs temperature
                 !            rnf_tsc(:,:,jp_tem) = sst_m(:,:) * rnf(:,:) * r1_rau0
            rnf_tsc(:,:,jp_tem) = MAX( sst_m(:,:), 0._wp ) * rnf(:,:) * r1_rau0 !Yona - normalement non utilisé car ln_rnf_tem=true
         ENDIF
         !                                                           ! use runoffs salinity data
   !! chris !Yona modif Christian to mask and call lbc_lnk but a priori not used because ln_rnf_sal=false
         IF( ln_rnf_sal )   THEN
            rnf_tsc(:,:,jp_sal) = ( sf_s_rnf(1)%fnow(:,:,1) * tmask(:,:,1) ) * rnf(:,:) * r1_rau0
            CALL lbc_lnk( rnf_tsc(:,:,jp_sal), 'T',  1. )
         ENDIF
   !! chris

         !                                                           ! else use S=0 for runoffs (done one for all in the init)
                                         CALL iom_put( 'runoffs'     , rnf(:,:)                         )   ! output runoff mass flux ( runoff +iceberg )
