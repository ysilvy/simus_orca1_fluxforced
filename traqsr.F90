MODULE traqsr
   ...
   INTEGER , PUBLIC ::   nn_chldta    !: use Chlorophyll data (=1/2/3) or not (=0)
   ...
   REAL(wp) , ALLOCATABLE, SAVE, DIMENSION(:,:) :: zpar_varsw  ! PAR fraction of shortwave
   ...
   SUBROUTINE tra_qsr( kt )
      ...
      ELSE                                        !  Ocean alone :
         ...
         IF( ln_qsr_rgb) THEN                             !  R-G-B  light penetration !
            ...
            IF( nn_chldta == 1 .OR. nn_chldta == 2 .OR. nn_chldta == 3 .OR. lk_vvl ) THEN  !Yona  !*  Variable Chlorophyll or ocean volume
               !
...
!CDIR NOVERRCHK
                  ...
                  END DO
                  !
               ELSEIF( nn_chldta == 3 ) THEN   !Yona     !*  3D Variable Chlorophyll read
                  !
                  CALL fld_read( kt, 1, sf_chl )            ! Read Chl data and provides it at the current time step
                  DO jk = 1, nksr + 1
                     zchl3d(:,:,jk) = sf_chl(1)%fnow(:,:,jk)
                  ENDDO
                  !
               ...
               zpar_varsw(:,:) = ( sf_par(1)%fnow(:,:,1) ) / 3.0
...
!!$               ze3(:,:,1) = zcoef * qsr(:,:)
!!$               zea(:,:,1) =         qsr(:,:)
               ze0(:,:,1) = ( 1. - 3. * zpar_varsw(:,:) ) * qsr(:,:)
               ze1(:,:,1) =             zpar_varsw(:,:)   * qsr(:,:)
               ze2(:,:,1) =             zpar_varsw(:,:)   * qsr(:,:)
...
!!$                        zchl = MIN( 10. , MAX( 0.03, zchl3d(ji,jj,jk) ) )
                        zchl = MIN( 10. , MAX( 0.05, zchl3d(ji,jj,jk) ) ) !! clem: 0.05 matches with bio !Yona
...
!!$                        zchl = MIN( 10. , MAX( 0.03, zchl3d(ji,jj,1) ) )
                        zchl = MIN( 10. , MAX( 0.05, zchl3d(ji,jj,1) ) ) !!clem: 0.05 matches with bio !Yona
                        irgb = NINT( 41 + 20.*LOG10(zchl) + 1.e-15 )
                        zekb(ji,jj) = rkrgb(1,irgb)
                        zekg(ji,jj) = rkrgb(2,irgb)
                        zekr(ji,jj) = rkrgb(3,irgb)
...
!!$                        zc3 = zcoef  * EXP( - fse3t(ji,jj,1) * zekr(ji,jj) )
                        !!clem !Yona
                        zc0 = ( 1. - 3. * zpar_varsw(ji,jj) ) * EXP( - fse3t(ji,jj,1) * xsi0r     )
                        zc1 =             zpar_varsw(ji,jj)   * EXP( - fse3t(ji,jj,1) * zekb(ji,jj) )
                        zc2 =             zpar_varsw(ji,jj)   * EXP( - fse3t(ji,jj,1) * zekg(ji,jj) )
                        zc3 =             zpar_varsw(ji,jj)   * EXP( - fse3t(ji,jj,1) * zekr(ji,jj) )
   ...
   SUBROUTINE tra_qsr_init
      ...
      TYPE(FLD_N)        ::   sn_par   ! clem: informations about the fields to be read !Yona
      !!
      NAMELIST/namtra_qsr/  sn_chl, sn_par, cn_dir, ln_traqsr, ln_qsr_rgb, ln_qsr_2bd, ln_qsr_bio, ln_qsr_ice,  &
...
902   IF( ios /= 0 ) CALL ctl_nam ( ios , 'namtra_qsr in configuration namelist', lwp )
      ...
      IF(lwp) THEN                ! control print
         ...
         WRITE(numout,*) '      RGB: Chl data (=1/2/3) or cst value (=0) nn_chldta  = ', nn_chldta
      ...
      IF( ln_traqsr ) THEN
         ...
         IF(lwp) WRITE(numout,*) '    ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~'
         !
         ALLOCATE( zpar_varsw(jpi,jpj) )
         !
         ALLOCATE( sf_par(1), STAT=ierror )           !* allocate and fill sf_par
         IF( ierror > 0 )   CALL ctl_stop( 'STOP', 'tra_qsr_init: unable to allocate sf_par structure' )
         !
         CALL fld_fill( sf_par, (/ sn_par /), cn_dir, 'tra_qsr_init', 'Variable PAR fraction ', 'namtra_qsr' )
      ...
      IF( ln_traqsr ) THEN     ! control consistency
         ...
         IF( ln_qsr_rgb .AND. nn_chldta == 3 )   nqsr =  4 !Yona
         IF( ln_qsr_2bd                      )   nqsr =  5
         IF( ln_qsr_bio                      )   nqsr =  6
         !
         IF(lwp) THEN                   ! Print the choice
            ...
            IF( nqsr ==  3 )   WRITE(numout,*) '         R-G-B   light penetration - 3D Chl data reconstructed'
            IF( nqsr ==  4 )   WRITE(numout,*) '         R-G-B   light penetration - 3D Chl data read'
            IF( nqsr ==  5 )   WRITE(numout,*) '         2 bands light penetration'
            IF( nqsr ==  6 )   WRITE(numout,*) '         bio-model light penetration'
         ENDIF
         !
      ENDIF
      !                          ! ===================================== !
      IF( ln_traqsr  ) THEN      !  Initialisation of Light Penetration  !
         ...
         IF( ln_qsr_rgb ) THEN            !  Red-Green-Blue light penetration  !
            ...
            !
            IF( nn_chldta == 1  .OR. nn_chldta == 2 .OR. nn_chldta == 3 ) THEN     !Yona      !* Chl data : set sf_chl structure
               IF(lwp) WRITE(numout,*)
