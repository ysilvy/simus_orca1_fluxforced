MODULE eosbn2
   ...
   END INTERFACE
   !
   INTERFACE eos_pt_from_ct
      MODULE PROCEDURE eos_pt_from_ct_2d, eos_pt_from_ct_3d
   END INTERFACE
   !
...
CONTAINS
   ...
   SUBROUTINE eos_insitu( pts, prd, pdep )
      ...
      !!clem: constrain temperature to be above freezing zfrp
      REAL(wp), DIMENSION(jpi,jpj,jpk) ::   zfrp
      !!----------------------------------------------------------------------
      !
      IF( nn_timing == 1 )   CALL timing_start('eos-insitu')
      !
      !!clem: freezing point temperature (celsius)
      zfrp(:,:,:) = 0.
      CALL eos_fzp( pts(:,:,:,jp_sal), zfrp(:,:,:), pdep(:,:,:) )
      !!clem
         ...
         DO jk = 1, jpkm1
            DO jj = 1, jpj
               DO ji = 1, jpi
                  ...
                  !!clem                 zt  = pts (ji,jj,jk,jp_tem) * r1_T0    ! temperature
                  zt  = MAX( zfrp(ji,jj,jk), pts (ji,jj,jk,jp_tem) ) * r1_T0    ! temperature
      ...
      CASE( 1 )                !==  simplified EOS  ==!
         ...
         DO jk = 1, jpkm1
            DO jj = 1, jpj
               DO ji = 1, jpi
                  !!clem               zt  = pts  (ji,jj,jk,jp_tem) - 10._wp
                  zt  = MAX( zfrp(ji,jj,jk), pts(ji,jj,jk,jp_tem) ) - 10._wp
               ...
               DO jj = 1, jpj
                  DO ji = 1, jpi
                     ...
                     DO jsmp = 1, nn_sto_eos*2
                        jdof   = (jsmp + 1) / 2
                        zh     = pdep(ji,jj,jk) * r1_Z0                                  ! depth
                        !!clem                        zt     = (pts (ji,jj,jk,jp_tem) + pts_ran(ji,jj,jk,jp_tem,jdof) * zsign(jsmp)) * r1_T0    ! temperature
                        zt     = (MAX(zfrp(ji,jj,jk), pts(ji,jj,jk,jp_tem)) + pts_ran(ji,jj,jk,jp_tem,jdof) * zsign(jsmp)) * r1_T0 ! temperature
   ...
   SUBROUTINE eos_insitu_2d( pts, pdep, prd )
      ...
      REAL(wp) ::   zn , zn0, zn1, zn2, zn3   !   -      -
      !!clem: constrain temperature to be above freezing zfrp
      REAL(wp), DIMENSION(jpi,jpj) ::   zfrp
      !!----------------------------------------------------------------------
      !
      ...
      CASE( -1, 0 )                !==  polynomial TEOS-10 / EOS-80 ==!
         ...
         DO jj = 1, jpjm1
            DO ji = 1, fs_jpim1   ! vector opt.
               ...
               !!clem               zt  = pts (ji,jj,jp_tem) * r1_T0                           ! temperature
               zt  = MAX( zfrp(ji,jj), pts (ji,jj,jp_tem) ) * r1_T0                           ! temperature
      ...
      CASE( 1 )                !==  simplified EOS  ==!
         ...
         DO jj = 1, jpjm1
            DO ji = 1, fs_jpim1   ! vector opt.
               !
               !!clem               zt    = pts  (ji,jj,jp_tem)  - 10._wp
               zt    = MAX( zfrp(ji,jj), pts  (ji,jj,jp_tem) )  - 10._wp
   ...
   SUBROUTINE rab_3d( pts, pab )
      ...
      !!clem: freezing point temperature (celsius)
      zfrp(:,:,:) = 0.
      CALL eos_fzp( pts(:,:,:,jp_sal), zfrp(:,:,:), fsdept(:,:,:) )
      !!clem
      ...
      CASE( 1 )                  !==  simplified EOS  ==!
         ...
         DO jk = 1, jpkm1
            DO jj = 1, jpj
               DO ji = 1, jpi
                  !!clem                  zt  = pts (ji,jj,jk,jp_tem) - 10._wp   ! pot. temperature anomaly (t-T0)
                  zt  = MAX( zfrp(ji,jj,jk), pts (ji,jj,jk,jp_tem) ) - 10._wp   ! pot. temperature anomaly (t-T0)
   ...
   SUBROUTINE rab_2d( pts, pdep, pab )
      ...
      CASE( 1 )                  !==  simplified EOS  ==!
         ...
         DO jj = 1, jpj
            DO ji = 1, jpi
               !
               !!clem               zt    = pts  (ji,jj,jp_tem) - 10._wp   ! pot. temperature anomaly (t-T0)
               zt    = MAX( zfrp(ji,jj), pts  (ji,jj,jp_tem) ) - 10._wp   ! pot. temperature anomaly (t-T0)
   ...
   SUBROUTINE rab_0d( pts, pdep, pab )
      ...
      CASE( -1, 0 )                !==  polynomial TEOS-10 / EOS-80 ==!
         ...
         !
         zh  = pdep * r1_Z0                                  ! depth
         !!clem         zt  = pts (jp_tem) * r1_T0                           ! temperature
         zt  = MAX( zfrp, pts (jp_tem) ) * r1_T0                           ! temperature
      ...
      CASE( 1 )                  !==  simplified EOS  ==!
         !
         !!clem         zt    = pts(jp_tem) - 10._wp   ! pot. temperature anomaly (t-T0)
         zt    = MAX( zfrp, pts(jp_tem) ) - 10._wp   ! pot. temperature anomaly (t-T0)
      ...
      DO jk = 2, jpkm1           ! interior points only (2=< jk =< jpkm1 )
         DO jj = 1, jpj          ! surface and bottom value set to zero one for all in istate.F90
            DO ji = 1, jpi
               ...
               !
               !!clem               pn2(ji,jj,jk) = grav * (  zaw * ( pts(ji,jj,jk-1,jp_tem) - pts(ji,jj,jk,jp_tem) )     &
               !!clem                  &                    - zbw * ( pts(ji,jj,jk-1,jp_sal) - pts(ji,jj,jk,jp_sal) )  )  &
               pn2(ji,jj,jk) = grav * (  zaw * ( MAX( zfrp(ji,jj,jk-1), pts(ji,jj,jk-1,jp_tem) )     &
                  &                            - MAX( zfrp(ji,jj,jk  ), pts(ji,jj,jk  ,jp_tem) ) )   &
   ...
   FUNCTION eos_pt_from_ct_2d( ctmp, psal ) RESULT( ptmp )
      !!----------------------------------------------------------------------
      ...
      !!----------------------------------------------------------------------
      !
      IF ( nn_timing == 1 )   CALL timing_start('eos_pt_from_ct_2d')
      !
      zdeltaS = 5._wp
      ...
      END DO
      !
      IF( nn_timing == 1 )   CALL timing_stop('eos_pt_from_ct_2d')
      !
   END FUNCTION eos_pt_from_ct_2d
   ...
   FUNCTION eos_pt_from_ct_3d( ctmp, psal ) RESULT( ptmp )
      ...
      !!                Rational approximation to TEOS10 algorithm (rms error on WOA13 values: 4.0e-5 degC)
      !!----------------------------------------------------------------------
      REAL(wp), DIMENSION(jpi,jpj,jpk), INTENT(in) ::   ctmp   ! Cons. Temp [Celcius]
      REAL(wp), DIMENSION(jpi,jpj,jpk), INTENT(in) ::   psal   ! salinity   [psu]
      ! Leave result array automatic rather than making explicitly allocated
      REAL(wp), DIMENSION(jpi,jpj,jpk) ::   ptmp   ! potential temperature [Celcius]
      !
      INTEGER  ::   ji, jj, jk           ! dummy loop indices
      REAL(wp) ::   zt , zs , ztm        ! local scalars
      REAL(wp) ::   zn , zd              ! local scalars
      ...
      !!----------------------------------------------------------------------
      !
      IF ( nn_timing == 1 )   CALL timing_start('eos_pt_from_ct_3d')
      !
      zdeltaS = 5._wp
      z1_S0   = 0.875_wp/35.16504_wp
      z1_T0   = 1._wp/40._wp
      ...
      DO jk = 1, jpkm1
         DO jj = 1, jpj
            DO ji = 1, jpi
               ...
               zs  = SQRT( ABS( psal(ji,jj,jk) + zdeltaS ) * r1_S0 )
               ztm = tmask(ji,jj,jk)
               !
               zn = ((((-2.1385727895e-01_wp*zt   &
                  &   - 2.7674419971e-01_wp*zs+1.0728094330_wp)*zt   &
      ...
      END DO
      !
      IF( nn_timing == 1 )   CALL timing_stop('eos_pt_from_ct_3d')
      !
   ...
   SUBROUTINE  eos_fzp_3d( psal, ptf, pdep )
      ...
      !!----------------------------------------------------------------------
      REAL(wp), DIMENSION(jpi,jpj,jpk), INTENT(in   )           ::   psal   ! salinity   [psu]
      REAL(wp), DIMENSION(jpi,jpj,jpk), INTENT(in   ), OPTIONAL ::   pdep   ! depth      [m]
      REAL(wp), DIMENSION(jpi,jpj,jpk), INTENT(out  )           ::   ptf    ! freezing temperature [Celcius]
      !
      INTEGER  ::   ji, jj, jk   ! dummy loop indices
      ...
      CASE ( -1, 1 )                !==  CT,SA (TEOS-10 formulation) ==!
         ...
         DO jk = 1, jpk
            DO jj = 1, jpj
               DO ji = 1, jpi
                  zs= SQRT( ABS( psal(ji,jj,jk) ) / 35.16504_wp )           ! square root salinity
   ...
   SUBROUTINE eos_pen( pts, pab_pe, ppen )
      ...
      CASE( 1 )                !==  Vallis (2006) simplified EOS  ==!
         ...
         DO jk = 1, jpkm1
            DO jj = 1, jpj
               DO ji = 1, jpi
                  !!clem                  zt  = pts(ji,jj,jk,jp_tem) - 10._wp  ! temperature anomaly (t-T0)
                  zt  = MAX( zfrp(ji,jj,jk), pts(ji,jj,jk,jp_tem) ) - 10._wp  ! temperature anomaly (t-T0)
                  zs = pts (ji,jj,jk,jp_sal) - 35._wp  ! abs. salinity anomaly (s-S0)
                  zh  = fsdept(ji,jj,jk)               ! depth in meters  at t-point
                  ztm = tmask(ji,jj,jk)                ! tmask
